...
#    def api_query(self, command, req, start, end):
    def api_query(self, command, req={}):
        if(command == "returnTicker" or command == "return24hVolume"):
            ...
            ret = urllib.request.urlopen(urllib.request.Request('https://poloniex.com/public?command=' + "returnTradeHistory" + '&currencyPair=' + str(req['currencyPair']) + "&start="+str(req['start']) + "&end="+str(req['end']))) 
            return json.loads(ret.read())
        elif(command == "returnChartData"):
            ret = urllib.request.urlopen(urllib.request.Request("https://poloniex.com/public?command=returnChartData&currencyPair=" + str(req['currencyPair']) + "&start=000000000&end=9999999999&period=300")) 
            return json.loads(ret.read())
        else:
            req['command'] = command
